...
export class ServiceConnectionClient extends AzureDevOpsClient {
  private static serviceType = "serviceConnection";
  private organizationName: string;
  private projectName: string;

  constructor(organizationName: string, projectName: string, credentials: ServiceClientCredentials) {
      super(credentials);
  ...
  public async createGitHubServiceConnection(endpointName: string, gitHubPat: string) {
      return this.serviceClient.sendRequest<any>(<UrlBasedRequestPrepareOptions>{
          url: this.getBaseOrgUrl(this.organizationName, ServiceConnectionClient.serviceType) + `/${this.projectName}/_apis/serviceendpoint/endpoints`,
          headers: {
  ...
  public async getEndpointStatus(endpointId: string): Promise<any> {
      return this.serviceClient.sendRequest<any>(<UrlBasedRequestPrepareOptions>{
          url: this.getBaseOrgUrl(this.organizationName, ServiceConnectionClient.serviceType) + `/${this.projectName}/_apis/serviceendpoint/endpoints/${endpointId}`,
          headers: {
              "Content-Type": "application/json",
              "Accept": "application/json;api-version=5.1-preview.2;excludeUrls=true"
          },
  ...
  public async authorizeEndpointForAllPipelines(endpointId: string): Promise<any> {
      return this.serviceClient.sendRequest<any>(<UrlBasedRequestPrepareOptions>{
          url: this.getBaseOrgUrl(this.organizationName, ServiceConnectionClient.serviceType) + `${this.projectName}/_apis/pipelines/pipelinePermissions/endpoint/${endpointId}`,
          headers: {
              "Content-Type": "application/json",
