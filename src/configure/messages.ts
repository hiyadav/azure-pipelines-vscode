export class Messages {
    ...
    public static branchRemoteMissing: string = `The branch: %s does not have any tracking branch and the selected repository has no remotes. Hence, we are unable to create a remote tracking branch. Kindly, create a remote tracking branch to procceed.`;
    ...
    public static remoteRepositoryNotConfigured: string = 'Remote repository is not configured. <a href="https://git-scm.com/docs/git-remote#Documentation/git-remote.txt-emaddem">Manage</a> your git repository with Azure Repos or Github';
